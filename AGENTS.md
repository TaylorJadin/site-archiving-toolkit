...
- The TUI uses the alt-screen and needs a real interactive TTY — you can't drive it through a plain piped shell. To demo/test it, run `./archive` in an actual terminal (e.g. via the Desktop) and press `enter` to start (URLs can be space- or newline-separated; `shift+enter` / `ctrl+j` insert a new line); the non-interactive subcommands (`quit`, `server ...`, `help`) work fine from a normal shell.
