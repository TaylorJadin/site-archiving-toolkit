...
- Crawls run in a background `archive session-run` subprocess. Press `d` to detach, `esc`/`c` to stop (also stops the Docker container). `./archive` reattaches to an active session; incomplete sessions can be resumed with `r` or `./archive resume`.
